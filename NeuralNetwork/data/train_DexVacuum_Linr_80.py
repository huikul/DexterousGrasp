...
parser.add_argument('--lr_sgd', type=float, default=0.0005)
...
parser.add_argument('--depth', default=80, type=int, help='layer of CNN')
...
if args.arch == "resnet_linr":
    model = ResidualNet_Linr('ImageNet',
                             ...
                             r_drop=args.r_drop).to(args.device)
'''
...
optimizer_sgd = torch.optim.SGD(model.parameters(), args.lr_sgd,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
''''''
optimizer_adam = optim.Adam(model.parameters(), lr=args.lr_adam)

scheduler_sgd = StepLR(optimizer_sgd, step_size=10, gamma=0.9)
...
def train(model, loader, epoch):
    ...
    for batch_idx, (data, score, label) in enumerate(loader):
        ...
        #
        data, score, label = data.to(args.device), \
                             (score.view(-1, 1)).to(args.device), \
