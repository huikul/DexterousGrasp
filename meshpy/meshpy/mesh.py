...
try:
    ...
    pass
    # logging.warning('Failed to import torch, GPU acceleration is unavialable')
...
class Mesh3D(object):
    ...
    def __init__(self, vertices, triangles, normals=None,
        ...
        # [self.min_gap_, self.max_gap_] = self._compute_min_max_gap()
        [self.min_gap_, self.max_gap_] = [None, None]
    ...
    @property
    def min_gap(self):
        """return the minmize distance between the neighbor points
        Note: Edited by Hui Zhang
        """
        return self.min_gap_

    @property
    def max_gap(self):
        """return the maxmize distance between the neighbor points
        Note: Edited by Hui Zhang
        """
        return self.max_gap_
    ...
    def compute_vertex_normals(self):
        """ Get normals from triangles
            EDITED BY HUI ZHANG
            for each vertex, the normal is estimated by the vectors that are extracted from its neighbor triangles.
            Considering each vertex belongs to more than one triangle, the final normal value is a weighted average
            normal.
        """
        normals = []
    ...
    def ray_intersections(self, ray, point, distance):
        ...
        for i, tri in enumerate(self.triangles):
            ...
            if (t > 0 and t < distance):
                # Hui: contact_point is the cross point between ray and the plane
                # where the triangle is located
                ...
                if Mesh3D._point_in_tri(tri_verts, contact_point):
                    ...
                    tri_point_pairs.append(contact_point)
        return tri_point_pairs


    def ray_intersections_fast(self, ray, point, distance):
        """
        Edited by Hui Zhang
        :param ray:                 1*3 float array
        :param point:               1*3 float array
        :param distance:            float
        :return:
                                    [] or n*3 float array -> cross points
        """
        ray = ray.reshape([1, 3])
        point = point.reshape([1, 3])

        ray = ray / np.linalg.norm(ray)         # 1*3
        norms = self.tri_normals()              # n*3
        [num_tri, tmp] = norms.shape

        tri_point_pairs = []
        shadow = np.sum(ray * norms, axis=1).reshape([num_tri, 1])    # n*1

        cond_1 = shadow[:, 0] > 0.0
        cond_2 = shadow[:, 0] < 0.0
        cond_sum = np.logical_or(cond_1, cond_2)
        pos = np.argwhere(cond_sum == True).squeeze()
        if len(pos) == 0:
            return tri_point_pairs

        shadow = shadow[pos, :]
        tris = self.triangles[pos, :]
        norms = norms[pos, :]
        # n*1
        t = np.sum((self.vertices[tris[:, 0]] - point) * norms[:, 0:3],
                   axis=1).reshape([len(pos), 1])
        t = t / shadow

        cond_1 = t[:, 0] > 0
        cond_2 = t[:, 0] < distance
        cond_sum = np.logical_and(cond_1, cond_2)

        if len(cond_sum) > 0:
            ind_cross = np.argwhere(cond_sum == True).squeeze()
            contact_points = point + t[ind_cross, :] * ray               # m*3

            tri_verts = np.zeros([contact_points.shape[0], 9])           # m*9
            tri_verts[:, 0:3] = self.vertices[tris[ind_cross, 0]]
            tri_verts[:, 3:6] = self.vertices[tris[ind_cross, 1]]
            tri_verts[:, 6:9] = self.vertices[tris[ind_cross, 2]]

            flgs_in_tri = Mesh3D._point_in_tri_fast(tri_verts, contact_points)
            pos_in_tri = np.argwhere(flgs_in_tri[:]==True).squeeze()
            # m*1
            if len(pos_in_tri) > 0:
                tri_point_pairs = contact_points[pos_in_tri, :]
        return tri_point_pairs

    def ray_intersections_GPU(self, ray, point, distance):
        """
        Edited by Hui Zhang

        :param ray:                 1*3 float array    MUST normalized before input
        :param point:               1*3 float array
        :param distance:            float
        :return:
                                    list  len==1  on GPU
        """
        tri_point_pairs = []
        ray = torch.from_numpy(ray.reshape([1, 3])).cuda()
        point = torch.from_numpy(point.reshape([1, 3])).cuda()
        # define properties of the 3D mesh on GPU
        norms = torch.from_numpy(self.tri_normals()).cuda()
        tris = torch.from_numpy(self.triangles).cuda()
        vertices = torch.from_numpy(self.vertices).cuda()
        [num_tri, tmp] = norms.shape

        # define constants on GPU
        torch_0 = torch.zeros(1).cuda()
        torch_distance = torch.tensor([distance], dtype=torch.float64).cuda()
        torch_true = torch.tensor([True], dtype=torch.bool).cuda()
        torch_false = torch.tensor([False], dtype=torch.bool).cuda()
        #
        shadow = torch.sum(ray * norms, axis=1).reshape([num_tri, 1])   # n*1
        cond_1 = shadow[:, 0] > torch_0
        cond_2 = shadow[:, 0] < torch_0
        cond_sum = torch.logical_or(cond_1, cond_2)
        pos = torch.where(cond_sum[:] == torch_true)[0]
        if pos.shape[0] == 0:
            return tri_point_pairs
        ...
        norms = norms[pos, :]
        # n*1
        t = torch.sum((vertices[tris[:, 0]] - point) * norms[:, 0:3],
                      axis=1).reshape([pos.shape[0], 1])
        t = t / shadow

        cond_1 = t[:, 0] > torch_0
        cond_2 = t[:, 0] < torch_distance
        cond_sum = torch.logical_and(cond_1, cond_2)

        if cond_sum.shape[0] > 0:
            ind_cross = torch.where(cond_sum[:] == torch_true)[0]
            contact_points = point + t[ind_cross, :] * ray               # m*3

            tri_verts = torch.zeros([contact_points.shape[0], 9]).cuda()   # m*9
            tri_verts[:, 0:3] = vertices[tris[ind_cross, 0]]
            tri_verts[:, 3:6] = vertices[tris[ind_cross, 1]]
            tri_verts[:, 6:9] = vertices[tris[ind_cross, 2]]

            flgs_in_tri = Mesh3D._point_in_tri_GPU(tri_verts, contact_points)
            pos_in_tri = torch.where(flgs_in_tri[:]==torch_true)[0]
            # m*1
            if pos_in_tri.shape[0] > 0:
                tri_point_pairs.append(contact_points[pos_in_tri, :])

        return tri_point_pairs

    def pts_intersections(self, pts, directions, max_dis):
        """
        Edited by Hui
        Calculate the nearest intersection for each point in pts
        :param directions:      n*3 float array:    must be normalized
        :param pts:             n*3 float array:    start points
        :param max_dis:         float:              max projects distance
        :return:
                                bool:               if successful
                                n*3 float array:    intersections or start point
        """
        pts_out = np.zeros([pts.shape[0], 3])
        for i in range(pts.shape[0]):
            tri_point_pairs = self.ray_intersections_fast(directions[i, :], pts[i, :], max_dis)
            if len(tri_point_pairs) == 0:
                pts_out[i, :] = 1.0 * pts[i, :]
            else:
                tri_point_pairs = tri_point_pairs.reshape([tri_point_pairs.size//3,3])
                tmp_dis = np.linalg.norm(tri_point_pairs[:, :]-pts[i, :].reshape([1, 3]), axis=1)
                ind_min = np.argmin(tmp_dis)
                pts_out[i, :] = 1.0 * tri_point_pairs[ind_min, :]
        return True, pts_out
    ...
    def rescale(self, scale_factor=1.0):
        ...
        """
        # recall the function -> @vertices.setter
        self.vertices = scale_factor * self.vertices
        self.center_of_mass_ = scale_factor * self._compute_com_uniform()
    ...
    ##################################################################
    def _compute_min_max_gap(self):
        """
        Compute the min/max gap between the neighbor vertex
        Edited by Hui Zhang
        :return:    float
        """
        min_gap = 9999999.0
        max_gap = -999999.0
        for i in range(self.triangles_.shape[0]):
            p_0 = self.vertices_[self.triangles_[0]]
            p_1 = self.vertices_[self.triangles_[1]]
            p_2 = self.vertices_[self.triangles_[2]]
            dist_0 = np.linalg.norm(p_0 - p_1)
            dist_1 = np.linalg.norm(p_0 - p_2)
            dist_2 = np.linalg.norm(p_2 - p_1)
            min_gap = np.min([min_gap, dist_0, dist_1, dist_2])
            max_gap = np.max([max_gap, dist_0, dist_1, dist_2])
    ...
    def _point_in_tri(tri_verts, point):
        ...
        # Compute Barycentric Coords (gravity center)
    ...
    @staticmethod
    def _point_in_tri_fast(tri_verts, point):
        """Is the given point contained in the given triangle?
        Note: reference -> def _point_in_tri(tri_verts, point)
        Parameters
        ----------
        tri_verts:          n*9 float array
        point:              n*1 float array
        Returns
        -------
        bool
            True if the point is in the triangle, False otherwise.
        ...
        # Compute vectors
        [num, vect] = tri_verts.shape
        v0 = tri_verts[:, 3:6] - tri_verts[:, 0:3]              # n*3
        v1 = tri_verts[:, 6:9] - tri_verts[:, 0:3]
        v2 = point - tri_verts[:, 0:3]
        # Compute Dot Products
        dot00 = np.sum(v0 * v0, axis=1).reshape([num, 1])       # n*1
        dot01 = np.sum(v0 * v1, axis=1).reshape([num, 1])
        dot02 = np.sum(v0 * v2, axis=1).reshape([num, 1])
        dot11 = np.sum(v1 * v1, axis=1).reshape([num, 1])
        dot12 = np.sum(v1 * v2, axis=1).reshape([num, 1])
        # Compute Barycentric Coords (gravity center)
        inv_denom = 1.0 / (dot00 * dot11 - dot01 * dot01)       # n*1
        u = (dot11 * dot02 - dot01 * dot12) * inv_denom         # n*1
        v = (dot00 * dot12 - dot01 * dot02) * inv_denom         # n*1
        ...
        cond_2 = (u + v)[:, 0] < 1 + 1e-5
        # Check if point is in triangle
        cond_sum = np.logical_and(cond_0, cond_1)
        cond_sum = np.logical_and(cond_sum, cond_2)
        return cond_sum

    @staticmethod
    def _point_in_tri_GPU(tri_verts, point):
        """Is the given point contained in the given triangle?
        Note: reference -> def _point_in_tri(tri_verts, point)
        Parameters
        ----------
        tri_verts:          n*9 float tensor    on GPU
        point:              n*1 float array     on GPU
        Returns
        -------
        bool
            True if the point is in the triangle, False otherwise.    on GPU
        """
        # Implementation provided by http://blackpawn.com/texts/pointinpoly/
        torch_1 = torch.tensor([1.0]).cuda()
        torch_var_0 = torch.tensor([-1e-5]).cuda()
        torch_var_1 = torch.tensor([1 + 1e-5]).cuda()
        # Compute vectors
        [num, vect] = tri_verts.shape
        v0 = tri_verts[:, 3:6] - tri_verts[:, 0:3]              # n*3
        v1 = tri_verts[:, 6:9] - tri_verts[:, 0:3]
        v2 = point - tri_verts[:, 0:3]
        # Compute Dot Products
        dot00 = torch.sum(v0 * v0, axis=1).reshape([num, 1])       # n*1
        dot01 = torch.sum(v0 * v1, axis=1).reshape([num, 1])
        dot02 = torch.sum(v0 * v2, axis=1).reshape([num, 1])
        dot11 = torch.sum(v1 * v1, axis=1).reshape([num, 1])
        dot12 = torch.sum(v1 * v2, axis=1).reshape([num, 1])
        # Compute Barycentric Coords (gravity center)
        inv_denom = torch_1 / (dot00 * dot11 - dot01 * dot01)       # n*1
        u = (dot11 * dot02 - dot01 * dot12) * inv_denom         # n*1
        v = (dot00 * dot12 - dot01 * dot02) * inv_denom         # n*1
        ...
        cond_0 = u[:, 0] > torch_var_0
        cond_1 = v[:, 0] > torch_var_0
        cond_2 = (u + v)[:, 0] < torch_var_1
        # Check if point is in triangle
        cond_sum = torch.logical_and(cond_0, cond_1)
        cond_sum = torch.logical_and(cond_sum, cond_2)
        return cond_sum
