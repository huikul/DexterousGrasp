...
"""
Modifiey be: Hui Zhang
Email      : hui.zhang@kuleuven.be
Date       : 01/03/2020 22:07
"""
...
class Sdf():
    ...
    # add by Hui Zhang
    @property
    def surface_thresh(self):
        return self.surface_thresh_
    ##################################################################
...
class Sdf3D(Sdf):
    ...
    def __init__(self, sdf_data, origin, resolution, use_abs=False,
        ...
        self.surface_for_signed_val = surface
    ...
    def estimate_normals(self, coords):
        """
        EDITED BY HUI ZHANG
        Returns the surface normal at the given coordinates,
        if the point is not on the surface, the surface normal will be estimated by the nearest surface point.
        ...
        -------
        float 3*1 array
            surface normal (always outwards the surface)
        Raises
        ------
        IndexError
            If the coords vector does not have three entries.
        ...
        if np.abs(sdf_val) < self.surface_thresh_:
            normal = self.surface_normal(coords)
        else:
            logging.debug('The point is not on surface')
            # find cloest surface point
            surface = self.surface_for_signed_val
            closest_surface_coord = surface[np.argmin(np.linalg.norm(surface - coords, axis=-1))]
            pt_grid = self.transform_pt_obj_to_grid(closest_surface_coord)
            normal = self.surface_normal(pt_grid)
    ...
    def get_signed_distances(self, pts_grid):
        """
        Edited by HUI ZHANG
        FUNCTION: estimate the signed distances for the input points in the grid.
        :param pts_grid:        3*n float array         a set points
        :return:
        """
        # print("sadfsdfsdf")
        dists = np.zeros(pts_grid.shape[1])
        ''''''
        for cnt_pt in range(0, pts_grid.shape[1]):
            dists[cnt_pt] = self._signed_distance(pts_grid[:, cnt_pt])
    ...
    def surface_normal(self, coords, delta=1.5):
        ...
        if self.is_out_of_bounds(coords):
            logging.debug('Out of bounds access. Snapping to SDF dims')
            logging.info('Out of bounds access. Snapping to SDF dims')
        ...
        if np.abs(sdf_val) >= self.surface_thresh_:
            ...
            logging.info('Cannot compute normal. Point must be on surface')
            return None
        ...
        if self[coords + n * 0.005] < self[coords]:
            n = -n
        return n

    def _compute_surface_points(self):
        # 在初始化函数中 self.resolution_ = resolution
        # self.surface_thresh_ = self.resolution_ * np.sqrt(2) / 2
        surface_points = np.where(np.abs(self.data_) < self.surface_thresh_)
        x = surface_points[0]
        y = surface_points[1]
        z = surface_points[2]
        # surface_ points 表面点在三维空间中坐标 存储格式如下：
        """
          [x1, y1, z1],
          [x2, y2, z2],
          [x3, y3, z3],
